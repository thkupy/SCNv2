...
if __name__ == '__main__':
    if int(sys.argv[1])==1:#do experiment
        ...
        PL=np.array(mylist)
        A_inrate=np.zeros((represR,represO))
        B_inrate=np.zeros((represR,represO))
        Ax_rate=np.zeros((represR,represO))
        Ax_rate_s=np.zeros((represR,represO))
        So_rate=np.zeros((represR,represO))
        So_rate_s=np.zeros((represR,represO))
        for iR in range(represR):
            for iO in range(represO):
                theseRepInds = np.logical_and(PL[:,0]==Na[iR],PL[:,1]==Onset[iO])
                A_inrate[iR,iO]= np.mean(A_inrate_raw[theseRepInds])
                B_inrate[iR,iO]= np.mean(B_inrate_raw[theseRepInds])
                Ax_rate[iR,iO] = np.mean(Ax_rate_raw[theseRepInds])
                Ax_rate_s[iR,iO] = np.std(Ax_rate_raw[theseRepInds])
                So_rate[iR,iO] = np.mean(So_rate_raw[theseRepInds])
                So_rate_s[iR,iO] = np.std(So_rate_raw[theseRepInds])
        s={}
        s['rep']=rep
        s['represR']=represR
        s['represO']=represO
        ...
        np.save(path_on_machine + 'Python/SCN/scnres6.npy',s)
    else:
        try:
            s = np.load(path_on_machine + 'Python/SCN/scnres6.npy')
            s = s.tolist()
            middleR=s['middleR']
            middleO=s['middleO']
        except:
    ...
    CAx=plt.contourf(s['Onset'],s['Na']-s['Nb'],ndimage.gaussian_filter(s['Ax_rate'],sigma=gauss_sigma,order=0),ncontours)
    ...
    #plt.tight_layout()
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres6_F1.pdf')
       
    plt.figure()
    CSo=plt.contourf(s['Onset'],s['Na']-s['Nb'],ndimage.gaussian_filter(s['So_rate'],sigma=gauss_sigma,order=0),ncontours)
    plt.ylabel('Apical - Basal active synapse count')
    plt.xlabel('Apical lead time (ms)')
    ...
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres6_F2.pdf')
    
    plt.figure()
    CA=plt.contourf(s['Onset'],s['Na']-s['Nb'],ndimage.gaussian_filter(s['A_inrate'],sigma=gauss_sigma,order=0),ncontours)
    plt.ylabel('Apical - Basal active synapse count')
    plt.xlabel('Apical lead time (ms)')
    ...
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres6_F3.pdf')
    
    plt.figure()
    CB=plt.contourf(s['Onset'],s['Na']-s['Nb'],ndimage.gaussian_filter(s['B_inrate'],sigma=gauss_sigma,order=0),ncontours)
    plt.ylabel('Apical - Basal active synapse count')
    plt.xlabel('Apical lead time (ms)')
    ...
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres6_F4.pdf')
    ...
    plt.subplot(122)
    plt.errorbar(s['Na']-s['Nb'],s['Ax_rate'][:,middleR],yerr=s['Ax_rate_s'][:,middleR],color='b',marker='o')
    plt.errorbar(s['Na']-s['Nb'],s['So_rate'][:,middleR],yerr=s['So_rate_s'][:,middleR],color='r',marker='o')
    ...
    plt.tight_layout()
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres6_F5.pdf')
