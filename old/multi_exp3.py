...
"Active Synapse count" vs. "Primary Dendrite Length" 2D plots for the SCN project
@author: kuenzel
"""
#
...
def myjob(myitems):
    ...
    R=runmodel(tstop=1000.0,dt=dt,a_on=True,b_on=True,inpn=(int(round(Na)),int(round(Nb))),
                                    starta=200.0,startb=200.0,seed=seed,
                                    pNeurit_L=myitems[0])
...
if __name__ == '__main__':
    if int(sys.argv[1])==1:#do experiment
        ...
        middleL = 1
        middleO = 2
        alllengths = np.linspace(1,125,represL)
        syncount = np.linspace(0,20,represO)
        Nrep = range(rep)      
        mylist = list(itertools.product(alllengths,syncount,Nrep))
        #
        start = time.time()
        ...
        for iL in range(represL):
            for iO in range(represO):
                theseRepInds = np.logical_and(PL[:,0]==alllengths[iL],PL[:,1]==syncount[iO])
        ...
        np.save(path_on_machine + 'Python/SCN/scnres12.npy',s)
    else:
        try:
            s = np.load(path_on_machine + 'Python/SCN/scnres12.npy')
            s = s.tolist()
            middleL=s['middleL']
            middleO=s['middleO']
            represR=s['represL']
...
#------plot part for exp6 begins here-----------
    ...
    #CAx=sp1.contourf(s['Na']-s['Nb'],s['pNeurite_L'],ndimage.gaussian_filter(s['Ax_rate'],
    #                 sigma=gauss_sigma,order=0),ncontours,cmap='inferno')
    #CAx=sp1.pcolormesh(s['Na']-s['Nb'],s['pNeurite_L'],ndimage.gaussian_filter(s['Ax_rate'],
                     #sigma=gauss_sigma,order=0),cmap='inferno')
    CAx=sp1.pcolormesh(np.flip(ndimage.gaussian_filter(s['Ax_rate'],
                     sigma=gauss_sigma,order=0),1),cmap='inferno')
    sp1.set_yticks(np.linspace(0.5,s['represL']-0.5,6))
    sp1.set_yticklabels((1,25,50,75,100,125))
    sp1.set_xticks(np.linspace(0.5,s['represO']-0.5,5))
    sp1.set_xticklabels(np.linspace(-20,20,5))
    SR=s['Na']-s['Nb']
    PL=s['pNeurite_L']
    AR=np.flip(ndimage.gaussian_filter(s['Ax_rate'],sigma=gauss_sigma,order=0),1)
    #AR=s['Ax_rate']
    imax=np.argmax(AR,1)
    imin=np.argmin(s['Ax_rate'],1)
    rmax=np.max(AR,1)
    rmin=np.min(AR,1)
    sp1.plot(imax+1,np.arange(s['represL'])+0.5,'ko',markersize=5,markerfacecolor='w')
    plt.ylabel(u'pNeurite L [µm]')
    plt.xlabel('Active synapse count (apical-basal)')
    ...
    sp2.plot(PL,rmax-rmin,'ko-',markerfacecolor='w')
    plt.xlabel(u'pNeurite L [µm]')
    plt.ylabel(r'max-min output rate ($\Delta$AP/s)')
    plt.title('Dynamic output range')
    plt.tight_layout()
    ...
    print(rmin)
    for iii in range(21):
        print('max ' + str(AR[iii,imax[iii]]) + '+/-' + str(str(s['Ax_rate_s'][iii,imax[iii]])))
        print('max ' + str(s['Ax_rate'][iii,imin[iii]]) + '+/-' + str(str(s['Ax_rate_s'][iii,imin[iii]])))
        print('---')
    
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres12_F1.pdf')
        
    plt.figure()
    sp3=plt.subplot(1,2,1)
    CSo=sp3.contourf(s['Na']-s['Nb'],s['pNeurite_L'],ndimage.gaussian_filter(s['So_rate'],
    ...
    if not machine_has_display:
        plt.savefig(path_on_machine + 'Python/SCN/scnres12_F2.pdf')
    else:
        plt.show()
#    plt.figure()
#    plt.subplot(221)
#    plt.errorbar(s['Na']-s['Nb'],s['Ax_rate'][4,:],yerr=(s['Ax_rate_s'][4,:]/np.sqrt(s['rep']-1)),
#                 color=[.0,.0,1.0],marker='o')
#    plt.errorbar(s['Na']-s['Nb'],s['Ax_rate'][7,:],yerr=(s['Ax_rate_s'][7,:]/np.sqrt(s['rep']-1)),
#                 color=[.3,.3,1.0],marker='o')
#    plt.errorbar(s['Onset'],s['Ax_rate'][11,:],yerr=(s['Ax_rate_s'][11,:]/np.sqrt(s['rep']-1)),
#                 color=[.5,.5,1.0],marker='o')
#    plt.errorbar(s['Onset'],s['Ax_rate'][14,:],yerr=(s['Ax_rate_s'][14,:]/np.sqrt(s['rep']-1)),
#                 color=[.7,.7,1.0],marker='o')
#    plt.xlabel('Apical lead time (ms)')
#    plt.ylabel('Rate AP/s')
#    plt.title('Onset delays vs. pNeurit_L')
#    plt.legend((u'25µm',u'50µm',u'75µm',u'100µm'))
#    plt.subplot(222)
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,0],yerr=(s['Ax_rate_s'][:,0]/np.sqrt(s['rep']-1)),
#                 color=[.0,.0,1.0],marker='o')
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,5],yerr=(s['Ax_rate_s'][:,5]/np.sqrt(s['rep']-1)),
#                 color=[.3,.3,1.0],marker='o')
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,10],yerr=(s['Ax_rate_s'][:,10]/np.sqrt(s['rep']-1)),
#                 color=[.5,.5,1.0],marker='o')
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,15],yerr=(s['Ax_rate_s'][:,15]/np.sqrt(s['rep']-1)),
#                 color=[.7,.7,1.0],marker='o')
#    plt.ylabel(u'pNeurite L [µm] vs. Onset delays')
#    plt.ylabel('Rate AP/s')
#    plt.title('Input rates')
#    plt.legend(('-100ms','-50ms','0ms','+50ms'))
#
#    plt.subplot(223)
#
#    plt.errorbar(s['Onset'],s['Ax_rate'][4,:]/s['Ax_rate'][4,10],
#                 yerr=(s['Ax_rate_s'][4,:]/s['Ax_rate'][4,10]/np.sqrt(s['rep']-1)),
#                 color=[.0,.0,1.0],marker='o')
#    plt.errorbar(s['Onset'],s['Ax_rate'][7,:]/s['Ax_rate'][7,10],
#                 yerr=(s['Ax_rate_s'][7,:]/s['Ax_rate'][7,10]/np.sqrt(s['rep']-1)),
#                 color=[.3,.3,1.0],marker='o')
#    plt.errorbar(s['Onset'],s['Ax_rate'][11,:]/s['Ax_rate'][11,10],
#                 yerr=(s['Ax_rate_s'][11,:]/s['Ax_rate'][11,10]/np.sqrt(s['rep']-1)),
#                 color=[.5,.5,1.0],marker='o')
#    plt.errorbar(s['Onset'],s['Ax_rate'][14,:]/s['Ax_rate'][14,10],
#                 yerr=(s['Ax_rate_s'][14,:]/s['Ax_rate'][14,10]/np.sqrt(s['rep']-1)),
#                 color=[.7,.7,1.0],marker='o')
#    plt.xlabel('Apical lead time (ms)')
#    plt.ylabel('Rate AP/s')
#    plt.title('Onset delays vs. pNeurit_L (normalized)')
#    plt.legend((u'25µm',u'50µm',u'75µm',u'100µm'))
#    plt.subplot(224)
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,0]/s['Ax_rate'][0,0],
#                 yerr=(s['Ax_rate_s'][:,0]/s['Ax_rate'][0,0]/np.sqrt(s['rep']-1)),
#                 color=[.0,.0,1.0],marker='o')
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,5]/s['Ax_rate'][0,5],
#                 yerr=(s['Ax_rate_s'][:,5]/s['Ax_rate'][0,5]/np.sqrt(s['rep']-1)),
#                 color=[.3,.3,1.0],marker='o')
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,10]/s['Ax_rate'][0,10],
#                 yerr=(s['Ax_rate_s'][:,10]/s['Ax_rate'][0,10]/np.sqrt(s['rep']-1)),
#                 color=[.5,.5,1.0],marker='o')
#    plt.errorbar(s['pNeurite_L'],s['Ax_rate'][:,15]/s['Ax_rate'][0,15],
#                 yerr=(s['Ax_rate_s'][:,15]/s['Ax_rate'][0,15]/np.sqrt(s['rep']-1)),
#                 color=[.7,.7,1.0],marker='o')
#    plt.title(u'pNeurite L [µm] vs. Onset delays (normalized)')
#    plt.ylabel('Rate AP/s')
#    plt.xlabel(u'pNeurit_L (µm)')
#    plt.legend(('-100ms','-50ms','0ms','+50ms'))
#
#
#    if not machine_has_display:
