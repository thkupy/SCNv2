...
if __name__ == '__main__':
    ...
    dt = 0.025
    rep = 10
    allintervals = np.linspace(10,100,36)
    bar = ChargingBar('Simulation running', max=rep*len(allintervals)*3 )
    if int(sys.argv[1])==1:#do experiment
        Aspikerate_b=np.zeros((len(allintervals),rep))
        Sspikerate_b=np.zeros((len(allintervals),rep))
        Aspikerate_a=np.zeros((len(allintervals),rep))
        Sspikerate_a=np.zeros((len(allintervals),rep))
        Aspikerate_ab=np.zeros((len(allintervals),rep))
        Sspikerate_ab=np.zeros((len(allintervals),rep))
        res={}
        for icond,thisitv in enumerate(allintervals):
            rAspikerate_b=[]
            rSspikerate_b=[]
            rAspikerate_a=[]
            rSspikerate_a=[]
            ...
            for thisrep in range(rep):
                ...
                Rb=runmodel(tstop=tstop,dt=dt,a_on=False, b_on=True,seed=3443268+(thisrep*100)+icond,
                           inpn=(1,20),starta=0.0,startb=0.0,syntau=2,synw_a=0.002,synw_b=0.002,
                           itv_a=20,itv_b=thisitv)
                ...
                Ra=runmodel(tstop=tstop,dt=dt,a_on=True, b_on=False,seed=3443268+(thisrep*100)+icond,
                           inpn=(20,1),starta=0.0,startb=0.0,syntau=2,synw_a=0.002,synw_b=0.002,
                           itv_a=thisitv,itv_b=20)
        ...
        np.save(path_on_machine + 'Python/SCN/scnres9.npy',res)
        bar.finish()
    else:
        try:
            res = np.load(path_on_machine + 'Python/SCN/scnres9.npy')
            res = res.tolist()
        except:
    ...
    plt.suptitle('Axon/Soma Spikerate vs. mean input interval (1/input rate)')
    ax1=plt.subplot(121)
    plt.vlines(1000.0/20.0,0,140,color='k',linestyle=':')
    plt.title('Axon Spikes')
    plt.ylabel('Spikerate (AP/s)')
    plt.xlabel('Mean input rate per input (Hz)')
    plt.errorbar(1000.0/res['intervals'],np.mean(res['Aspikerate_a'],axis=1),yerr=np.std(res['Aspikerate_a'],axis=1),color='b',marker='o')
    plt.errorbar(1000.0/res['intervals'],np.mean(res['Aspikerate_ab'],axis=1),yerr=np.std(res['Aspikerate_ab'],axis=1),color='k',marker='o')
    plt.errorbar(1000.0/res['intervals'],np.mean(res['Aspikerate_b'],axis=1),yerr=np.std(res['Aspikerate_b'],axis=1),color='r',marker='o')
    plt.legend(('default','20 apical only','10 apical + 10 basal','20 basal only'),loc=2)
    ax2=plt.subplot(122)
    plt.vlines(1000.0/20.0,0,140,color='k',linestyle=':')
    plt.title('Soma Spikes')
    plt.xlabel('Mean input rate per input (Hz)')
    plt.errorbar(1000.0/res['intervals'],np.mean(res['Sspikerate_a'],axis=1),yerr=np.std(res['Sspikerate_a'],axis=1),color='b',marker='o')
    plt.errorbar(1000.0/res['intervals'],np.mean(res['Sspikerate_ab'],axis=1),yerr=np.std(res['Sspikerate_ab'],axis=1),color='k',marker='o')
    plt.errorbar(1000.0/res['intervals'],np.mean(res['Sspikerate_b'],axis=1),yerr=np.std(res['Sspikerate_b'],axis=1),color='r',marker='o')
    plt.tight_layout()    
    plt.show()
