...
This file contains the 9th experiment set of the SCN model project.
Input-Output relation of apical, basal and apical+basal synapses.
Part 3: Length of the "primary neurite" part.
@author: kuenzel
"""
#
...
def runmodel(tstop=1000,dt=0.025,a_on=True, b_on=True,seed=32768,
    ...
    Soma.nseg = 7
    #
    pNeurit.L = pNeurit_L#variable (default = 45um; from table)
...
if __name__ == '__main__':
    ...
    rep = 20
    conditions = 36
    alllengths = np.linspace(1,125,conditions)
    bar = ChargingBar('Simulation running', max=rep*conditions*3 )
    if int(sys.argv[1])==1:#do experiment
        Aspikerate_b=np.zeros((conditions,rep))
        Sspikerate_b=np.zeros((conditions,rep))
        Aspikerate_a=np.zeros((conditions,rep))
        Sspikerate_a=np.zeros((conditions,rep))
        Aspikerate_ab=np.zeros((conditions,rep))
        Sspikerate_ab=np.zeros((conditions,rep))
        res={}
        for icond,thislen in enumerate(alllengths):
            rAspikerate_b=[]
            rSspikerate_b=[]
            rAspikerate_a=[]
            rSspikerate_a=[]
        ...
        np.save(path_on_machine + 'Python/SCN/scnres10.npy',res)
        bar.finish()
    else:
        try:
            res = np.load(path_on_machine + 'Python/SCN/scnres10.npy')
            res = res.tolist()
        except:
    ...
    plt.suptitle('Axon/Soma Spikerate vs. primary neurite length (soma to axon)')
    ax1=plt.subplot(121)
    plt.vlines(1000.0/20.0,0,140,color='k',linestyle=':')
    plt.title('Axon Spikes')
    plt.ylabel('Spikerate (AP/s)')
    plt.xlabel(u'Primary neurite length (µm)')
    plt.errorbar(res['lengths'],np.mean(res['Aspikerate_a'],axis=1),yerr=np.std(res['Aspikerate_a'],axis=1),color='b',marker='o')
    plt.errorbar(res['lengths'],np.mean(res['Aspikerate_ab'],axis=1),yerr=np.std(res['Aspikerate_ab'],axis=1),color='k',marker='o')
    plt.errorbar(res['lengths'],np.mean(res['Aspikerate_b'],axis=1),yerr=np.std(res['Aspikerate_b'],axis=1),color='r',marker='o')
    plt.legend(('default','20 apical only','10 apical + 10 basal','20 basal only'),loc=2)
    ax2=plt.subplot(122)
    plt.vlines(1000.0/20.0,0,140,color='k',linestyle=':')
    plt.title('Soma Spikes')
    plt.xlabel(u'Primary neurite length (µm)')
    plt.errorbar(res['lengths'],np.mean(res['Sspikerate_a'],axis=1),yerr=np.std(res['Sspikerate_a'],axis=1),color='b',marker='o')
    plt.errorbar(res['lengths'],np.mean(res['Sspikerate_ab'],axis=1),yerr=np.std(res['Sspikerate_ab'],axis=1),color='k',marker='o')
    plt.errorbar(res['lengths'],np.mean(res['Sspikerate_b'],axis=1),yerr=np.std(res['Sspikerate_b'],axis=1),color='r',marker='o')
    plt.tight_layout()    
    plt.show()
