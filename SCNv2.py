...
def runmodel_prop(
        ...
        pstimulation=(0.0, 1, 5.0, 1.0, 5.0, 1, 5.0, 1.0),
        haselectrode=False,
        pelectrode=(0,10.0, 50.0, 0.2, 0.0),
    ...
    """
    Creates and runs the second version of the SCN model. However, this version
    is interested in closely observing the voltage propagation in the apical
    dendrites and soma. [new 12/2021]
    """ 
    #
    #--------MODEL DEFINITIONS---------
    #CREATE SECTIONS
    ...
    # INFRASTRUCTURE
    ###a bit stupid
    positions = np.linspace(0,1,nvecs)
    vm_apical = []
    vm_neurite = []
    vm_soma = []
    vm_basal = []
    for ivec in range(nvecs):
        vm_apical.append(h.Vector())
        vm_apical[ivec].record(aDend(positions[ivec])._ref_v)
        vm_neurite.append(h.Vector())
        vm_neurite[ivec].record(pNeurit(positions[ivec])._ref_v)
        vm_soma.append(h.Vector())
        vm_soma[ivec].record(Soma(positions[ivec])._ref_v)
        vm_basal.append(h.Vector())
        vm_basal[ivec].record(bproxDend(positions[ivec])._ref_v)
    #
    #----------SIMULATE--------------
    nrngo(tstop+pretime,-66.5)
    #
    # PACK AND EXPORT DATA
    all_vm_apical = np.zeros((int(tstop/dt),nvecs)) * np.nan
    all_vm_neurite = np.zeros((int(tstop/dt),nvecs)) * np.nan
    all_vm_soma = np.zeros((int(tstop/dt),nvecs)) * np.nan
    all_vm_basal = np.zeros((int(tstop/dt),nvecs)) * np.nan
    for ivec in range(nvecs):
        tva = vm_apical[ivec].as_numpy()
        all_vm_apical[:,ivec] = tva[int(pretime/dt):int((tstop+pretime)/dt)] #remove "additional" samples
        tvn = vm_neurite[ivec].as_numpy()
        all_vm_neurite[:,ivec] = tvn[int(pretime/dt):int((tstop+pretime)/dt)] #remove "additional" samples
        tvs = vm_soma[ivec].as_numpy()
        all_vm_soma[:,ivec] = tvs[int(pretime/dt):int((tstop+pretime)/dt)] #remove "additional" samples
        tvb = vm_basal[ivec].as_numpy()
        all_vm_basal[:,ivec] = tvb[int(pretime/dt):int((tstop+pretime)/dt)] #remove "additional" samples
    Result = {}
    Result['vm_apical'] = all_vm_apical
    Result['vm_neurite'] = all_vm_neurite
    Result['vm_soma'] = all_vm_soma
    Result['vm_basal'] = all_vm_basal
    return(Result)
