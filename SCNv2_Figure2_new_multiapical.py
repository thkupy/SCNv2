...
This creates the an addendum to the new Figure 2 (non-deterministic inputs
with realistic latencies) demanded by reviewer 2 during the submission to the
Journal of Neuroscience. What happens if all inputs (both "auditory" and "visual"
connections) are connected to the apical dendrite? This addresses the question
whether the morphology of the SCN plays a crucial, partial or no role in the
multimodal enhancement.
This function also loads data for the "normal" configuration
(SCNv2_Figure2_new.py) for comparison. Make sure this ran successful before you 
run this function here. A new comparison figure is produced as well.

The command line args are:
    SCNv2_Figure2_new_multiapical.py weload ncores ncond nreps
...
def runonecondition(x, P):
    ...
    ####(the spiketimes are always the same to improve comparability)
    if P["AdvSeed"]:
    ...
    for irep in range(P["nreps"][x]):
        thisR = SCNv2.runmodel(
            ...
            allapical=P["allapical"][x],
        )#new imput parameter 2021-11-17 (J Neurosci revision)
...
def plotres(outputA, outputB, outputC, PA, PB, PC):
    ...
    PA["N"]
    exI = (0, int(np.floor(PA["N"]/2)), int(PA["N"]-1))
...
def getparams(
            ...
            reallatency=True,
            allapical=False,#added 2021-11-17 (J Neurosci revision)
        ):
        ...
        P["allapical"] = np.repeat(allapical, nconds)#added 2021-11-17 (J Neurosci revision)
        return P


def plotres_comparison():
    '''
    This is the new function (2021-11-17) to show the comparison between the
    normal configuration and the "multimodal apical dendrite" the second reviewer
    of the J Neurosci wanted to see.
    '''
    #Load the results for the normal configuration
    ...
    c_s = outputC[0][:, 1]
    c_t = outputC[0][:, 2]
    #Load the results for the multimodal apical dendrite
    outputAm = np.load("./data/SCNv2_Figure2_new_multiapicalA.npy", allow_pickle=True)
    outputBm = np.load("./data/SCNv2_Figure2_new_multiapicalB.npy", allow_pickle=True)
    outputCm = np.load("./data/SCNv2_Figure2_new_multiapicalC.npy", allow_pickle=True)
    PAm = np.load("./data/SCNv2_Figure2_new_multiapicalPA.npy", allow_pickle=True)
    PAm = PAm.tolist()
    PBm = np.load("./data/SCNv2_Figure2_new_multiapicalPB.npy", allow_pickle=True)
    PBm = PBm.tolist()
    PCm = np.load("./data/SCNv2_Figure2_new_multiapicalPC.npy", allow_pickle=True)
    PCm = PCm.tolist()
    a_mm = outputAm[0][:, 0]
    a_sm = outputAm[0][:, 1]
    a_tm = outputAm[0][:, 2]
    ...
    fheight = 8  # cm
    fwidth = 9.25
    fhandle3 = plt.figure(figsize=(fwidth / 2.54, fheight / 2.54))#, dpi=600)
    #
    ...
    plt.xlabel("Mean Input Frequency (Hz)")
    plt.ylabel("Mean Output (APs)")
    plt.legend(("visual normal", "visual all apical"),fontsize=5)
    #
    sp2 = plt.subplot(2,2,2, sharey=sp1)
    sp2.set_xscale("log")
    ...
    plt.xlabel("Mean Input Frequency (Hz)")
    plt.ylabel("Mean Output (APs)")
    plt.legend(("auditory normal", "auditory all apical"),fontsize=5)
    #
    sp3 = plt.subplot(2,2,3, sharey=sp1)
    plt.plot(np.linspace(0,100,PC["N"]), a_m + b_m, "g-")
    plt.plot(np.linspace(0,100,PCm["N"]), a_mm + b_mm, "g--")
    plt.errorbar(
        np.linspace(0,100,PC["N"]), 
        c_m, 
        yerr=c_s,
    ...
    plt.ylabel("Mean Output (APs)")
    plt.legend((
        "$\Sigma$ unimodal normal", 
        "$\Sigma$ unimodal all apical",
        "multimodal normal", 
        "multimodal all apical",
        ),fontsize=4)
    #
    #
    ...
    plt.legend(("enhancement normal", "enhancement all apical"),fontsize=5)
    plt.xlabel(r"Input Salience $\%$")
    plt.ylabel("AP Increase (AP) \n vs. sum")
    #
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    #SCNv2_Saliencetests.py weload ncores nconds nreps --- this was the old name
    ...
    #------------------
    #Run the show
    #Filenames improved 2021-11-17 (J Neurosci revision)
    if os.path.isfile("./data/SCNv2_Figure2_new_multiapicalA.npy") and weload:
        print("Loading Data!")
        outputA = np.load("./data/SCNv2_Figure2_new_multiapicalA.npy", allow_pickle=True)
        outputB = np.load("./data/SCNv2_Figure2_new_multiapicalB.npy", allow_pickle=True)
        outputC = np.load("./data/SCNv2_Figure2_new_multiapicalC.npy", allow_pickle=True)
        PA = np.load("./data/SCNv2_Figure2_new_multiapicalPA.npy", allow_pickle=True)
        PA = PA.tolist()
        PB = np.load("./data/SCNv2_Figure2_new_multiapicalPB.npy", allow_pickle=True)
        PB = PB.tolist()
        PC = np.load("./data/SCNv2_Figure2_new_multiapicalPC.npy", allow_pickle=True)
        PC = PC.tolist()
    else:
        PA = getparams(
            ncores = ncores,
        ...
        np.save("./data/SCNv2_Figure2_new_multiapicalA.npy", outputA, allow_pickle=True)
        np.save("./data/SCNv2_Figure2_new_multiapicalPA.npy", PA, allow_pickle=True)
        outputB = []
        outputB.append(myMPhandler(PB))
        outputB = np.array(outputB, dtype=object)
        np.save("./data/SCNv2_Figure2_new_multiapicalB.npy", outputB, allow_pickle=True)
        np.save("./data/SCNv2_Figure2_new_multiapicalPB.npy", PB, allow_pickle=True)
        outputC = []
        outputC.append(myMPhandler(PC))
        outputC = np.array(outputC, dtype=object)
        np.save("./data/SCNv2_Figure2_new_multiapicalC.npy", outputC, allow_pickle=True)
        np.save("./data/SCNv2_Figure2_new_multiapicalPC.npy", PC, allow_pickle=True)
    #
    print("done")
