...
def plotres(outputA, outputB, PA, PB):
    ...
    a_bm = ndimage.gaussian_filter(a_bm, sigma=filtsig, order=0)
    a_abm = ndimage.gaussian_filter(a_abm, sigma=filtsig, order=0)
    b_am = ndimage.gaussian_filter(b_am, sigma=filtsig, order=0)
    b_bm = ndimage.gaussian_filter(b_bm, sigma=filtsig, order=0)
    b_abm = ndimage.gaussian_filter(b_abm, sigma=filtsig, order=0)
    #
    dotcol = ((.1, .1, .1), (.5, .5, .5))
    msize = 1
    dottype = ","
    ...
    c22 = sp22.pcolormesh(X, Y2, z22, cmap="Blues", shading="gouraud", vmin=0, vmax=30)
    plt.colorbar(c22, ax=sp22, extend="both")
    sp22.set_ylabel(" \n Apical tau-decay (ms)")
    cl22 = sp22.contour(X,Y2,z22,(5,10,15,20,25,30), colors="k", linewidths=0.5)
    sp22.clabel(cl22, cl22.levels, fmt="%1.0f", fontsize=6)
    #
    sp24 = plt.subplot(5,2,4)
    z24 = np.reshape(b_bm, (nconds,nfreqs))
    c24 = sp24.pcolormesh(X, Y2, z24, cmap="Reds", shading="gouraud", vmin=0, vmax=30)
    plt.colorbar(c24, ax=sp24, extend="both")
    sp24.set_ylabel(" \n Apical tau-decay (ms)")
    cl24 = sp24.contour(X,Y2,z24,(5,10,15,20,25,30), colors="k", linewidths=0.5)
    sp24.clabel(cl24, cl24.levels, fmt="%1.0f", fontsize=6)
    #
    sp26 = plt.subplot(5,2,6)
    z26 = np.reshape(b_abm, (nconds,nfreqs))
    c26 = sp26.pcolormesh(X, Y2, z26, cmap="bone", shading="gouraud", vmin=0, vmax=70)
    plt.colorbar(c26, ax=sp26, extend="both")
    sp26.set_ylabel(" \n Apical tau-decay (ms)")
    cl26 = sp26.contour(X,Y2,z26,(10,20,30,40,50,60,70), colors="k", linewidths=0.5)
    sp26.clabel(cl26, cl26.levels, fmt="%1.0f", fontsize=6)
    #
    sp28 = plt.subplot(5,2,8)
    z28 = np.reshape((b_am + b_bm), (nconds,nfreqs))
    c28 = sp28.pcolormesh(X, Y2, z28, cmap="Greens", shading="gouraud", vmin=0, vmax=70)
    plt.colorbar(c28, ax=sp28, extend="both")
    sp28.set_ylabel(" \n Apical tau-decay (ms)")
    cl28 = sp28.contour(X,Y2,z28,(10,20,30,40,50,60,70), colors="k", linewidths=0.5)
    sp28.clabel(cl28, cl28.levels, fmt="%1.0f", fontsize=6)
    #
    sp210 = plt.subplot(5,2,10)
    z210 = np.reshape(b_abm - (b_am + b_bm), (nconds,nfreqs))
    c210 = sp210.pcolormesh(X, Y2, z210, cmap="seismic", shading="gouraud", vmin=-30, vmax=50)
    plt.colorbar(c210, ax=sp210, extend="both")
    sp210.set_ylabel(" \n Apical tau-decay (ms)")
    sp210.set_xlabel("Input Power (\%)")
    cl210 = sp210.contour(X,Y2,z210,(10,20,30,40,50), colors="k", linewidths=0.5)
